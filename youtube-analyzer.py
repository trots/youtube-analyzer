...
class MainWindow(QMainWindow):
    def __init__(self, settings: Settings):
        ...
        btn = QToolButton()
        btn.setText("+")
        btn.clicked.connect(self._create_new_tab)
        self._main_tab_widget.setCornerWidget(btn, Qt.Corner.TopRightCorner)
    ...
    def _on_export_xlsx(self):
        ...
        if file_path:
            current_widget = self._main_tab_widget.currentWidget()
            export_to_xlsx(file_path, current_widget.model)

    def _on_export_csv(self):
        file_path = self._get_file_path_to_export(self.tr("Save CSV"), self.tr("Csv File (*.csv)"), ".csv")
        if file_path:
            current_widget = self._main_tab_widget.currentWidget()
            export_to_csv(file_path, current_widget.model)

    def _on_export_html(self):
        file_path = self._get_file_path_to_export(self.tr("Save HTML"), self.tr("Html File (*.html)"), ".html")
        if file_path:
            current_widget = self._main_tab_widget.currentWidget()
            export_to_html(file_path, current_widget.model)
    ...
    def _on_preferences(self):
        ...
        for tab_index in range(self._main_tab_widget.count()):
            tab_widget = self._main_tab_widget.widget(tab_index)
            tab_widget.handle_settings_change()
